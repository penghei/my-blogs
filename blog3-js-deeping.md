...
首先要明确一点，闭包不一定会导致内存泄漏。
内存泄漏的定义是什么？无用的对象在内存中依然占据位置。如果闭包的变量是一个我们在闭包内部确实有用的值，那就不应该被算作内存泄漏，只能算是较大的内存占用：
...
fn1()   //2
fn1()   //3
// 虽然可能不再需要sum变量了，但是仍有可能再调用fn1，因此如果还需用到，就不算泄漏
// 反之，如果确定不会用到了，我们可以手动删除fn1
fn1 = null
```

另外一种情况是，即使存在未使用的函数，但由于闭包共享作用域，闭包变量依然会被保存：
...
虽然someData从来没有被使用过，但依然会保存其引用。这种情况我们需要在函数myFn执行完后将闭包变量置null来解决。

这种情况本质上是代码的问题，而非闭包本身的问题。因此使用闭包时，只要保证闭包变量被正常使用，并且不会过大，就不会造成问题。
...
最基本的出现场景，不再赘述
但是需要注意的是，只要是在函数内部嵌套定义的，就会产生闭包，即使这个函数没有执行。
下面的例子中，unused函数虽然没有执行，但他在定义中引用了bar，因此还是会形成闭包。
