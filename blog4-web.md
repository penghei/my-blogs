...
- quic 的 frame 属于一个 stream，每个 stream 有自己的滑动窗口，在自己 stream 内部实现了流量控制，不会影响其他 stream
...
- TCP和UDP可以用同一个端口。端口只是一个应用层的入口，通过端口之后，具体的应用程序会来取走不同的报文，即使两者都用同一个也不影响。
...
2. 输出转码。防止浏览器执行恶意代码。这部分分为两类：

- 防止 HTML 中出现注入。
  - 禁止拼接 html，采用前端渲染的方式，
  - 对 html 进行转译。对于可能要插入 html 的地方，将特殊字符转义（参考下面）。当使用服务端返回的 html 直接渲染，或者通过 innerHTML 插入时，都需要这样的转译。
  > 转码之后的内容不会以html的形式显示，而是以文本形式存在。理论上原本就应该是文本形式，这里其实就是在预防攻击者插入恶意的`<script>`，因此把所有可能的字符全部转码。转码之后作为文本显示正常，但作为html则不能识别
- 防止 JavaScript 执行时，执行恶意代码。
  - 不要把不可信的数据作为 HTML 插到页面上，尽可能不使用 innerHTML 等方式，通过 innerText、setAttribute 等方式创建 html

3. 其他。比如为了防止 a 标签执行`javascript:`开头的代码，可以在拼接 href 时检查是否是以`http`或`https`开头的链接，不是就不允许执行。
