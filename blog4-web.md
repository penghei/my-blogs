...
- TCP 报文头没有「包长度」字段，UDP 有。原因是 TCP 的报文长度计算公式为`IP总长度 - IP首部长度 - TCP首部长度`，三者都知道，无需单独计算。
...
1. 消息边界分隔符：在消息中添加特定的分隔符，例如换行符、空格等，以便接收方可以根据分隔符来判断消息的边界。
...
- 防止 HTML 中出现注入。
  - 禁止拼接 html，采用前端渲染的方式，
  - 对 html 进行转译。对于可能要插入 html 的地方，将特殊字符转义（参考下面）
