...
八股留的时间比较多，因为可能上班还有事情要做，但是尽量抽时间复习就行。另外白天复习八股，晚上不能不做算法，就算不做hard也得做新题的mid，为的是保证自己有个“深度思考”的习惯和状态。记住这个千万不能偷懒，哪怕只思考半小时也行，做2道题也行，但是不能完全不做

7.17：复习html、css、js八股，全部重新过一遍。注意这次因为是完全重新复习，因此要全过，不能只看概要。算法做hard补充3-4道，即top100等题单里的hard题，最好总结记录一下，因为这些是可能出原题或类似题的。
7.18：复习web、浏览器、工程化和webpack八股，即前面7篇博客内容。算法补题，不一定是hard，可以补一些同样是top榜上的没做过的middle或easy，5道，最好把力扣top100和牛客top101补完
7.19：复习react和react native八股，包括react的两个、rn的一个。算法先不做，复习手写题，把手写题都过一遍。可以先完全不复习默写，然后不会的标记下来，重写一遍，周末再重写一次。
...
7.23：兜底，因为算法这个比较多，因此周末之内复习完就行。如果复习完的话可以再总过一遍八股，看看有没有疏漏。补充完新题，后续的可以从比如top100里、codetop等地方挑做过的题重新做；
