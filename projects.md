...
首先是优化会场页的 c 指标，即首屏加载速度
优化手段主要有：

- 分包，通过 RAMBundles + InlineRequire 进行分包，将庞大模块分离出去减小包体积。具体细节有：
  - 分包的对象
    - 可能不在首屏加载的模块，需要懒加载的模块。项目中是商详页的猜喜、榜单。这些模块采用懒加载，当他们进入可视区域时才会加载，因此需要对他们进行分包。及时这些模块有可能在首屏，但我们仍把它们看做是非核心模块，因此进行分包和懒加载。
    - 没有数据就不展示的模块，项目中是三种不同专区都可以分包，主接口包含专区数据时再加载和渲染。
    - 需要用户手动触发的模块，项目中就是手动触发的 Modal 弹窗。（但是这块有坑，怎么处理加载时的 pending 状态？）
