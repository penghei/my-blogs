...
- 一个bug，之前出现过播放时出现两个音轨的情况，主要原因是按索引匹配的时候出现了两个active的元素。这个在自行实现的时候可能能通过一些方式避免
- 
...
1. 样式适配问题。大部分样式的数值都需要rpx转换，写死的样式通常会有问题。即使是相同的样式，在不同机型上也会有不同的布局表现，在ios和安卓上也有不同表现。

这个不仅体现在样式的适配上，还有一些功能的适配。解决方法主要就是多做尝试，比如xcode切换不同机型，针对部分特殊机型做专门的测试，以及设计好兜底方式
更极端的，有时候样式还要区分ios和安卓。在细节样式的调试过程中经常会出现这种情况。

2. MRN对RN做了⼤量的优化⼯作，总结下来主要有三点：加载流程优化、运⾏环境隔离以及⼯程化保障。