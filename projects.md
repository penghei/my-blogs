...
- **预加载**：接入预加载能力，主要是从美团 app 首页到会场页的预加载，和在会场页对商详页的预加载。
  ...
  - 执行时机：选择在 tab 吸顶之后预加载。因为根据用户流向数据，吸顶之后进入商详页的比例较高，因此在 tab 吸顶之后开始执行商详页容器的预热和页面预加载操作
...
- 第一个难点是轮播图的实现和轮播组件的封装，以及在之后还需要在轮播组件的基础上加入对视频元素的支持。这部分的主要难点产生于轮播图的一些关键逻辑，比如无限滚动等在之前不了解；同时由于开发时对业务采用的 rn 技术栈还不够熟悉，也遇到了很多的 bug 和问题。并且在后面接入视频时我们需要让页面只维持一个视频组件，因此需要考虑如何把视频组件接入到轮播图内并能够做到维持单例。最后就是还要保证一定的性能，不能让轮播图占用过高的渲染消耗。
- 第二个难点是对页面的性能优化，主要难点在于性能问题的发现和接入方式。在优化首屏加载时间时，我没有按照合理的优化流程，而是根据之前的经验应用了一些基础优化，但发现效果很差。后面在接入优化的过程中，也产生了一些优化成本过高、效果不佳的问题，因此找到性能问题的准确来源并选择合适的优化方式就是优化过程的主要难点。

2. 超管项目：

- 主要难点在于使用 canvas 绘制地图，并实现和地图的交互功能。由于地图是业务自定义需求，因此需要自己开发地图功能。在这个过程中我选择了 konva 作为 canvas 框架。在使用过程中有一个需求是在一定范围内点击点位，需要获取点位的坐标等信息。这是一个常见的事件委托需求，但使用 konva 实现的时候发现了 konva 事件流和预想的不一样。我通过查文档、查 issues 等方式都没有找到合适的解决方案，最终通过调试和阅读源码的方式搞清楚了 konva 的事件和事件流机制，并解决了事件委托的需求。
- 在完成地图需求之后，我又对地图做了一些优化来保证性能。地图主要的性能问题是当点位数量变多时，拖拽会出现不流畅的感觉。我通过 performance 性能面板来对拖拽过程的帧数和长任务来做记录，发现了当点位数量比较多时，每次拖拽操作会导致页面帧数波动，下降到 50 帧左右。对此我还进行了一些优化。在优化过程中，因为 konva 本身已经对 canvas 的绘制过程做了一些优化，因此我先通过阅读文档和查看源码的方式了解了 konva 采用了哪些优化手段，去掉这些优化方式后，最后应用了一些专项的优化。优化之后可以保证当点位数量在正常取值范围内时，拖拽不会再出现大幅度掉帧的情况。

亮点

1. 特团项目：

- 在轮播图的实现中采用了 rn 自带的虚拟列表来优化轮播图，可以精确控制初始渲染数量和缓冲数量等参数，能达到更好的性能
- 在优化过程中采取了合理的流程，按照 DMAIC 的逻辑来去做优化，效果比直接堆优化方案要好。并且根据项目的实际情况和专项问题来做具体的优化

2. 超管项目：

- 使用了 konva 来实现了一个基础的地图功能，包括点位绘制、路径绘制等功能，并能实现交互
- 根据项目情况采用 recoil 作为项目的状态管理库，相对于 redux 来说使用起来跟项目更加契合
...
- 要实现的效果（循环、可滑动、自动播放、三张、视频播放）
- mrn 不支持直接引入 rn 或 web 组件，只能自己开发

2. 目标

- 开发并封装好可用的轮播组件，并对视频功能做一定的支持
- 利用优化手段维持性能稳定，不会造成渲染负担和内存负担
...
- 轮播图组件的开发和封装
  - 功能实现关键点
    - 基本切换功能、元素定位的实现
    - 无限滚动的实现
    - 容器：FlatList 和 ScrollView
  - 封装处理，大致记得几个方面：
    - api 设计，包括值规范、默认值、兜底和报警、单一职责、回调内外分离、可扩展性（renderItem）等
    - 分离和组合，比如把切换按钮分离出去
    - 纯净，不修改外部数据源
- 轮播图组件的初步优化
  - 利用 FlatList 的初始渲染数量、缓冲区大小来优化
  - 组件通过 memo 处理，组件内不使用 useState，FlatList 的 data 等参数通过 useMemo 处理
  - 商卡降低复杂度，用图片和兜底背景色代替大量的元素
- 轮播图接入视频
  - 初始方案：固定视频，移动视频
  - 实际方案：移动视频，及时销毁
  - 销毁方式：只有两个商卡组件更新，一个卸载 video，一个挂载 video

4. 结果

- 实现了轮播需求和视频需求
- 保证页面稳定性，尽量降低轮播图对页面渲染、内存的影响
...
按照 DMAIC 和 STAR 结合来说，可以夹杂下面的优化思想的内容
其实 DMAIC 本质也是一种 STAR 的思想

可以先说：项目优化是一个比较麻烦的过程，首先因为项目技术栈的特殊性，常规的 web 优化知识可能不能生效，因此优化前需要先了解一些专项的优化方式；其次是优化的每一个方法都应该规范化，以一套较为完善的方法来实现，包括性能问题的发现、优化前后的测量、优化之后的维持等等。因此我们按照一定的优化链路来对项目进行优化。

1. D：定义优化的对象和目标

- 对象：有两个，首屏加载速度提升，和解决闪烁卡顿问题
- 目标：
  - C 指标 2 -> 20，综合指标 40 -> 60
    - C指标的计算：FSP，首屏元素增长趋于稳定，兜底为屏幕填充率
    - 线上指标的计算：三种机型，每种机型按照百分位数排布加载时间，apdex计算分数（`(满意样本*1+可容忍样本*0.5+令人沮丧样本*0)/样本总数`）
  - 首屏加载时间，用于测试的两台机型和线上值
    - 安卓中端：3500 -> 2000 以内
    - ios14: 2400 -> 1800 以内
    - 线上：以评分为主，综合不同机型考虑
...
- 测量对象：
  ...
  - 各阶段的具体加载时间和占比
    - 业务包处理：5% + 10% + 20% = 35%
    - 业务接口请求时间：不固定，一般超过 30%
    - 渲染时长：包括 react 和 native，20%
  - 包的体积和结构
- 测量结果：
  - 以测试机为标准得到首屏时间平均值
  - 测量各阶段具体时间，得到最长的占比是业务包的编译执行、业务组件渲染和网络请求三部分
  - 包体积：980KB，其中解压后 js 占到 1MB 左右，静态资源 200KB 左右
  - 包的结构，各个库的占比

3. A：分析原因，根据测量结果猜想可能的原因并验证

- 业务包处理时间长：未做分包，未优化包体积，未做预加载
- 网络请求：时长不能改变，但未提前请求，并且没有缓存和预请求过程
- 渲染时长：未做懒加载、延迟渲染、分步渲染等，部分组件过于复杂可能会拖慢渲染速度
- 专项问题，包括滚动闪烁和吸顶卡顿
- 验证：分析之后还需要验证。验证的方法基本就是去查看数据和代码，看看是不是因为没有做这个地方而导致性能问题

4. I：实际操作

- 优化方案分两个阶段，一阶段上低成本优化，二阶段上较高成本的精细优化
- 优化具体方案的得出
  - 常规方式根据内部技术文档实现
  - 专项方式根据项目实际情况具体优化，比如代码分割的具体内容等
- 具体方案
  - 一阶段低成本优化
    - 包体积
      - 通过 cli 发现删除重复包和过时包
      - 引入 babel 插件优化路径导入，部分组件采取单体下载
    - 请求前置
    - 按需渲染
    - 专项问题解决
    - 轮播图渲染优化、视频离屏卸载等细节
    - 图片：选定合适分辨率、webp 格式、开启懒加载、
  - 二阶段精细优化
    - RAMBundles + require：tab 页分离、数据驱动模块分离、非首屏模块分离，还有初始包的整合
    - 分步渲染：原因、效果、细节问题
    - 预加载：吸顶后/空闲时预加载商详页和专区二级页，同时也预加载头图
- 优化效果
  - 包体积：900KB -> 750KB 左右
  - 请求前置：加载时间减少约 200ms
  - 代码分割：体积变化不大，首屏时间减少约 600ms
  - 预加载：二级页加载时间减少 200ms 以上，接近一半
- 最终结果：
  - 安卓机：3500 -> 2500
  - ios: 2400 -> 1800
  - C 指标：2 -> 25
...
- 首先我先了解了一些常规的 canvas 优化方式：
  - 绘制方面：分层、批量绘制、局部绘制、脏矩形、离屏渲染、时间分片
  - 计算方面：优化指令、合理赋值、降低计算开销、减少上下文修改次数
- 然后了解 konva 已经做的一些默认的优化方式，和它已经封装的一些优化手段，可以直接接入的内容
  - 时间分片
  - 优化指令和上下文改变的开销
  - 批量渲染
- 得到我们还可以做的内容，包括分层、局部绘制、离屏渲染等
- 最后结合业务实际情况，应用了一些优化手段，包括：
  - 局部绘制，当放缩和拖拽之后，重新绘制点位时根据点位坐标，超出可视区域的就不执行绘制。
  - 缩小不绘制：当缩小到一定程度时，位于点位下方的点位名称部分已经看不清楚，设置当缩小到一定比例时就不再绘制一部分内容，不影响观感的同时能减少渲染量
  - 离屏渲染：利用 konva 封装的机制对点位、路径等元素采用 cache 保存，每次只绘制它的克隆版本。

### Recoil

为什么采用 Recoil 作为状态管理方案？

可以从这几方面来说

- 首先先了解了常用的几种状态管理方案：redux、mobx、recoil、原生 context api
- 状态管理方案本身的优缺点
  - redux：使用广，可靠，规范程度高；模板代码太多，异步方案比较麻烦，状态集中导致杂乱，少量共享效率反而很低
  - mobx：仅次于 redux，同样可靠，响应式写法简单；响应式写法和 react 不搭，不方便调试，需要手写封装
  - recoil：简单易用，原子化状态，对 hooks 和 ts 支持好；全局唯一 key 需要维护心智，atom 数量多需要整理，对 ssr 支持不好，包体积较大
- 项目实际需要的场景
  - 多为较少组件间的管理，比如 drawer 和外部的 table，tab 和 list 等，以及嵌套的子孙组件中
  - 数据较零碎，不同组件间数据关系不大
  - 所以需要 recoil 这样的较为零碎的管理方案，同时能保证功能的使用
- 其他方面
  - recoil 使用方便，模板代码少，对 ts 支持好，采用 react 原生方案封装
