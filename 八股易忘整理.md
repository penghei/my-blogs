...
- HTTP版本
    - 1.1 缓存 长连接 字段和方法增加 | 队头阻塞
    - 2.0 帧和流 HPACK压缩 服务器推送 | TCP队头阻塞
    - 3.0 1RTT和0RTT 连接复用 解决TCP队头阻塞
- QUIC
    - 重要部分
        - packet：packetnumber，类比seq，自增（原因：乱序，防止阻塞）
...
- csrf攻击的防范：token、验证码、referer、samesite
- DDoS攻击
